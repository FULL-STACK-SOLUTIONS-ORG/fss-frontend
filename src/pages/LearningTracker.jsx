...
const LearningTracker = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center" style={{ paddingTop: '100px' }}>
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-teal-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-slate-300">Loading learning modules...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-900" style={{ paddingTop: '100px', paddingBottom: '48px' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {}
        <div className="mb-8 bg-slate-800/50 rounded-2xl p-6 border border-slate-700 backdrop-blur-sm">
          <div className="flex flex-col lg:flex-row gap-8">
            {}
            <div className="flex-1">
              <div className="text-center mb-4">
                <h2 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-teal-400 via-cyan-400 to-blue-400 bg-clip-text text-transparent inline-flex items-center gap-2">
                ...
                <div className="flex flex-col items-center justify-center gap-3 mt-4">
                  <div className="flex items-center gap-4">
                    <div className="bg-slate-900/80 px-4 py-1.5 rounded-full border border-orange-500/30 flex items-center gap-2 shadow-lg shadow-orange-500/10">
                      <span className="text-lg animate-pulse">🔥</span>
                      <span className="text-sm font-bold text-orange-400">
                        Streak: <span className="text-white">{(() => {
                    ...
                    {}
                    <div className="bg-slate-900/80 px-4 py-1.5 rounded-full border border-yellow-500/30 flex items-center gap-2 shadow-lg shadow-yellow-500/10">
                      <span className="text-lg"><AnimatedCoin className="w-5 h-5" textSize="text-[8px]" /></span>
                      <span className="text-sm font-bold text-yellow-400">
                        Points: <span className="text-white">{userPoints}</span>
                      </span>
                    </div>
                    {}
                    <div className="flex items-center gap-2 bg-slate-900/80 rounded-full px-3 py-1.5 border border-slate-700/50">
                      <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wider hidden sm:inline">DAILY TARGET</span>
                      <div className="flex items-center gap-1">
                        <button 
                          onClick={() => setDailyTarget(Math.max(1, dailyTarget - 1))}
                          className="w-5 h-5 rounded bg-slate-700 text-white flex items-center justify-center hover:bg-slate-600 text-xs transition-colors"
                        >-</button>
                        <span className="text-white font-bold w-5 text-center text-sm">{dailyTarget}</span>
                        <button 
                          onClick={() => setDailyTarget(Math.min(20, dailyTarget + 1))}
                          className="w-5 h-5 rounded bg-slate-700 text-white flex items-center justify-center hover:bg-slate-600 text-xs transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          disabled={dailyTarget >= 20}
                        >+</button>
                      </div>
                      <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wider hidden sm:inline">TOPICS</span>
                    </div>
                  </div>
                  {}
                        ...
                        return currentBadges.map((badge, idx) => {
                          ...
                          return (
                            <div key={idx} className="relative group">
                              {}
                              <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none border border-slate-700 z-50 shadow-xl">
                                <span className={`font-bold ${isUnlocked ? 'text-teal-400' : 'text-slate-400'}`}>{badge.fullName}</span>
                                <span className="text-slate-500 ml-1">({badge.threshold} pts)</span>
                              </div>
                              <div className={`w-10 h-10 rounded-lg flex items-center justify-center border-2 transition-all duration-300 ${
                                isUnlocked 
                                  ...
                                  : 'bg-slate-800/50 border-slate-700 opacity-20 grayscale'
                              }`}>
              ...
              <div className="relative px-10">
                <button 
                  ...
                  className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-slate-700 rounded-full p-2 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110 border border-slate-600 hover:border-teal-500"
                >
                  ...
                  return (
                    <div key={idx} className="flex flex-col items-center gap-1 min-w-[40px]">
                      <div className="relative w-9 h-9 flex items-center justify-center group">
                        ...
                        {}
                        <div className={`absolute inset-0 m-1 rounded-full flex items-center justify-center flex-col transition-colors ${day.isToday ? 'bg-slate-700/50' : 'group-hover:bg-slate-700/30'}`}>
                          <span className={`text-[10px] font-bold ${day.isToday ? 'text-white' : 'text-slate-400'}`}>
                            {day.date}
                          </span>
                        </div>
                        {isCompleted && (
                          <div className="absolute -right-1 -top-1 bg-green-500 rounded-full p-0.5 shadow-lg border border-slate-800">
                      ...
                      <span className={`text-[9px] font-bold uppercase tracking-wider ${day.isToday ? 'text-teal-400' : 'text-slate-500'}`}>
                        {day.dayName}
                      </span>
                    </div>
                  );
                ...
                <button 
                  onClick={() => setWeekOffset(prev => prev + 1)} 
                  className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-slate-700 rounded-full p-2 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110 border border-slate-600 hover:border-teal-500"
                >
            ...
            {}
            <div className="w-full lg:w-64 bg-slate-900/50 rounded-xl p-3 border border-slate-700 flex flex-col relative group">
              ...
              <div className="grid grid-cols-7 gap-1 text-center mb-1">
                {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                  <div key={i} className="text-[9px] font-bold text-slate-500">{d}</div>
                ))}
              </div>
                ...
                {(() => {
                  ...
                  for (let i = 1; i <= daysInMonth; i++) {
                    ...
                    if (count >= dailyTarget) {
                      bgClass = 'bg-green-500 text-white shadow-lg shadow-green-900/20'; 
                    } else if (count > 0) {
                      bgClass = 'bg-yellow-500/50 text-yellow-100'; 
                    }
                    const isToday = i === actualToday.getDate() && month === actualToday.getMonth() && year === actualToday.getFullYear();
                    days.push(
                      <div 
                        key={i} 
                        className={`aspect-square rounded-md flex items-center justify-center text-[10px] font-medium transition-all hover:scale-110 cursor-default ${bgClass} ${isToday ? 'ring-1 ring-teal-400 ring-offset-1 ring-offset-slate-900 z-10' : ''}`}
              ...
              <button 
                ...
                className="absolute -left-3 top-1/2 -translate-y-1/2 z-10 bg-slate-700 rounded-full p-1.5 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110 border border-slate-600 hover:border-teal-500 "
              >
              ...
              <button 
                onClick={() => setCalendarDate(new Date(calendarDate.getFullYear(), calendarDate.getMonth() + 1, 1))}
                className="absolute -right-3 top-1/2 -translate-y-1/2 z-10 bg-slate-700 rounded-full p-1.5 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110 border border-slate-600 hover:border-teal-500 "
              >
        ...
        {modules.length === 0 ? (
          <div className="bg-slate-800 rounded-xl shadow-md p-12 text-center">
            ...
            <p className="text-slate-300 text-lg font-semibold">No modules available</p>
            <p className="text-slate-400 text-sm mt-2">Modules will appear here once they are added by the admin</p>
          </div>
        ) : (
          <>
            {}
            ...
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-slate-700 rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110 border-2 border-slate-600 hover:border-teal-500"
            ...
            <button
              onClick={goToNext}
              className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-slate-700 rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110 border-2 border-slate-600 hover:border-teal-500"
            ...
            >
                  ...
                  return (
                    ...
                    >
                      <div 
                        ...
                        className={`bg-slate-800 rounded-2xl shadow-xl p-6 border-2 flex flex-col relative ${isModuleLocked ? 'border-amber-600' : 'border-slate-700'}`} 
                        style={{ height: '800px' }}
                      >
                        {}
                        {isModuleLocked && (
                          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm rounded-2xl z-10 flex flex-col items-center justify-center p-6">
                            ...
                            <h4 className="text-2xl font-bold text-white mb-2 text-center">Module Locked</h4>
                            ...
                            <div className="flex gap-3">
                              ...
                              <button
                                ...
                                className="px-6 py-3 bg-slate-700 text-white rounded-lg hover:bg-slate-600 transition-colors font-semibold border-2 border-slate-600"
                              >
                                Sign Up
                              </button>
                            </div>
                        ...
                        <div className="mb-6 flex-shrink-0">
                          <div 
                            className="bg-slate-900/50 rounded-xl p-4 border border-slate-600 shadow-inner backdrop-blur-sm relative overflow-hidden group hover:border-blue-500/50 transition-colors duration-300"
                          >
                            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"></div>
                            <h3 className="text-xl font-bold text-blue-300 text-center relative z-10">
                              <span className="text-blue-500">Module {index + 1} :</span> {module.title.replace(/^Module\s+\d+\s*[:\s-]+\s*/i, '')}
                            </h3>
                            {module.category && (
                              <p className="text-slate-400 text-xs text-center mt-1 relative z-10 uppercase tracking-wider font-semibold">
                                {module.category}
                              </p>
                            )}
                          </div>
                        ...
                        <div className="flex-1 flex flex-col min-h-0">
                              <div className="mb-4 flex-shrink-0">
                                ...
                                <div className="w-full rounded-full h-3 overflow-hidden bg-slate-600">
                                  <div
                                    className="bg-gradient-to-r from-teal-500 to-cyan-500 rounded-full h-3 transition-all duration-500 flex items-center justify-end pr-1.5"
                              ...
                              {}
                              <div className="relative bg-gradient-to-r from-amber-500/20 via-yellow-500/20 to-amber-500/20 border-2 border-amber-400 rounded-lg p-2.5 mb-4 flex items-center gap-2 flex-shrink-0 shadow-xl instruction-box">
                                <p className="text-amber-100 font-bold text-sm tracking-wide flex-1">
                                ...
                                return (
                                  <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                                          ...
                                          >
                                            <button
                                              ...
                                              className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 ${
                                                topic.completed
                                                  ...
                                                  : 'border bg-slate-600 border-slate-500'
                                              }`}
                                            ...
                                            {(topic.hasQuiz || topic.quizzes?.length > 0 || topic.quiz?.question) && (
                                              <button
                                                ...
                                                className={`ml-2 px-2 py-1 text-xs font-semibold text-white rounded-md transition-colors ${
                                                  isQuizCompleted
                                                    ? 'bg-green-600 hover:bg-green-500 shadow-md shadow-green-900/20'
                                                    : topic.completed 
                                    ...
                                    {practicalProblems.length > 0 && (
                                      <div className="mt-3 flex-shrink-0 bg-slate-900/40 rounded-xl p-3 border border-amber-500/20 shadow-inner">
                                            ...
                                            <div
                                              ...
                                              className={`flex items-center gap-3 p-2 rounded-lg transition-all duration-200 border ${
                                                topic.completed
                                                  ...
                                                  : 'bg-slate-800/30 border-slate-700 hover:border-amber-500/30'
                                              }`}
                                            >
                                              <button
                                                onClick={() => handleToggleTopic(module._id, topic._id, topic.completed, index)}
                                                className={`flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center transition-all duration-200 ${
                                                  topic.completed
                                                    ...
                                                    : 'border border-slate-500 bg-transparent hover:border-amber-400'
                                                }`}
                                                title={topic.completed ? 'Mark as incomplete' : 'Mark as complete'}
                                              >
                                                {topic.completed && (
                                              ...
                                              <div className="flex-1 min-w-0 flex items-center justify-between gap-2">
                                                <span className={`text-sm font-medium truncate ${topic.completed ? 'line-through text-slate-500' : 'text-slate-300'}`}>
                                                  {topic.name}
                                                </span>
                                                  ...
                                                  <a
                                                    ...
                                                    className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-semibold transition-all ${
                                                      topic.completed
                                                        ...
                                                        : 'bg-amber-500/10 text-amber-400 border border-amber-500/50 hover:bg-amber-500 hover:text-white'
                                                    }`}
              ...
              <div className="flex-shrink-0 w-full lg:w-[400px] lg:sticky lg:top-32">
                <div className="bg-slate-800 rounded-2xl shadow-xl p-6 border-2 border-slate-700 flex flex-col" style={{ height: '800px' }}>
                  <h3 className="text-lg font-bold text-white mb-4 text-center">Module Progress</h3>
                  <div className="flex flex-col items-center justify-center flex-1">
                    <PieChart
                      ...
                      showPercentage={true}
                    >
                      <span className="text-sm text-slate-300 mt-2 font-medium text-center">
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-700 w-full">
                      <div className="text-center">
                        <p className="text-sm text-slate-500 mb-1">Total Topics Completed</p>
                        ...
                        <p className="text-xs text-slate-400 mt-1">topics in all modules</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  className={`flex-shrink-0 transition-all duration-300 rounded-full flex items-center justify-center font-bold text-[10px] w-7 h-7 ${
                    index === currentIndex
                      ...
                      : 'bg-slate-700 text-slate-300 hover:bg-slate-600 hover:text-white hover:scale-105 border border-slate-600'
                  }`}
          ...
          <div className="max-w-4xl mx-auto mt-12 mb-8 px-4">
            <div className="bg-slate-800/80 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4">
               <div className="flex flex-col md:flex-row items-center gap-4 justify-between">
                  ...
                  {feedbackSuccess ? (
                     <div className="flex-1 text-center text-green-400 text-sm font-semibold animate-in fade-in">
                     ...
                     <div className="flex-1 w-full flex items-center gap-3">
                        ...
                        <button
                           ...
                           className="bg-teal-600 hover:bg-teal-500 text-white px-6 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap shadow-lg shadow-teal-900/20"
                        >
      ...
      {showDailyLimitModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-slate-800 rounded-2xl p-8 max-w-md w-full border border-slate-700 shadow-2xl transform scale-100 animate-in zoom-in-95 duration-200 relative overflow-hidden">
            {}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-32 bg-gradient-to-br from-orange-400 to-red-500 opacity-20 blur-3xl rounded-full"></div>
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <div className="text-slate-300 mb-6 space-y-3 text-left w-full">
...
const BadgeModal = ({ badge, isOpen, onClose, userName }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-slate-800 rounded-2xl p-8 max-w-sm w-full border border-slate-700 shadow-2xl transform scale-100 animate-in zoom-in-95 duration-200 relative overflow-hidden">
        {/* Background Glow */}
        <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-32 h-32 bg-gradient-to-br ${details.color} opacity-20 blur-3xl rounded-full`}></div>
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          ...
          <p className="text-slate-300 mb-6">{details.message}</p>
          
          <div className="bg-slate-900/50 rounded-xl p-4 w-full border border-slate-700/50 mb-4">
            <p className="text-xs text-slate-400 uppercase tracking-wider font-bold mb-1">Next Goal</p>
            <p className="text-teal-400 font-medium">{details.nextGoal}</p>
          </div>
          ...
          <div className="flex gap-3 w-full">
            <button
              onClick={onClose}
              className="flex-1 px-4 py-3 bg-slate-700 text-white rounded-xl font-bold hover:bg-slate-600 transition-all duration-200"
            >
              Close
            </button>
            <button
